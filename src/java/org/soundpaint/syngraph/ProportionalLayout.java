 ...
 * $Id: ProportionalLayout.java 307 2008-01-03 07:28:53Z reuter $
 */
...
class ProportionalLayout implements LayoutManager2
{
					       ...
					       int[] spans)
	{
	    ...
	    for (int i = 0; i < children.length; i++)
		{
		    ...
		    spans[i] = (int)Math.min ((long) ascent +
					      (long) descent,
					      Integer.MAX_VALUE);
		}     
	}
    ...
    {
	// FIXME: when the x size of target is smaller than preferred
	// x size, the y size currently screws up.
	checkContainer (target);
	...
	if (xTotal == null)
		 ...
		 *
		 * c_0 left bound coordinate (in pixels), c_1 right
		 * bound coordinate (in pixels), c_a proportional
		 * alignment in the range 0.0 (left aligned) to 1.0
		 * (right aligned), and c_t the (preferred) size of c.
		 *
		 * d_0, d_1, d_a, d_t be accordingly defined for a
		 * component d.
		 *
		 * t be the (horizontal) preferred size to determine.
		 *
		 * Note that c_0 = c_a * t - c_a * c_t, and c_1 = c_0
		 * + c_t (and accordingly for component d).  If (c_a
		 * == d_a), then c and d always collide, and we do not
		 * care about them, since we can not do anything
		 * against the collsision.  Otherwise, we assume that
		 * c_a < d_a (if not, we just flip c and d).  Then c
		 * does not horizontally collide with da if and only
		 * if
		 *
		 * d0 > c1 <=>
		 *
		 ...
		 *
		 * According to our assumption c_a < d_a, we are sure
		 * not to devide by zero.
		 *
		 * Since t has to be greater for all such components c
		 * and d with c_a < d_a, we have to compute the
		 * maximum of the above division for all such pairs
		 * (c, d).
		 *
		 * We currently implement here a simple algorithm with
		 * O(n^2) time (with n = number of components) for
		 * considering all pairs (c, d) with (c_a < d_a).
		 *
		 * Note that by prior sorting of the components by
		 * their alignment value with an O(n * log n)
		 * algorithm, a simple linear walk through the sorted
		 * list of components would deliver adjacent pairs
		 * (which is sufficient for collision detection) in
		 * effectively O(n) time.  Only if a component is
		 * added, the list of components would have to be
		 * updated to be again in sorted order.
		 */
